...
public class Intersection {
    ...
    public List<Route> getConnections() {
        List<Route> connections = new ArrayList<>(this.incomingConnections);
        connections.add(new Route("id", new Intersection("id"), 5));
        return connections;
    }
    ...
    public void reduceIncomingSpeedSigns() {
        for (Route route : incomingConnections) {
            ...
            if (currentSpeed >= 50) {
                route.setSpeedLimit((Math.max(50, currentSpeed - 10)));
            }
        }
    }
