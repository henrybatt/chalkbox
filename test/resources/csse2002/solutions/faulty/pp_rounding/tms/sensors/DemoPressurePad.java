...
public class DemoPressurePad extends DemoSensor implements PressurePad {
    ...
    public int getCongestion() {
        ...
        if ((float) congestionPct > (100 * congestion)) {
            return Math.min(Math.max((int) Math.floor(100 * congestion), 0), 100);
        } else {
            return Math.min(Math.max((int) Math.ceil(100 * congestion), 0), 100);
		}
    }
