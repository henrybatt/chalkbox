...
*/
        ...
        DemoSpeedCamera demoSpeedCamera = new DemoSpeedCamera(speedCameraData,100);
        DemoPressurePad demoPressurePad = new DemoPressurePad(speedCameraData,2);

        try {
            testNetwork.get(0).getConnection(testNetwork.get(2)).addSensor(demoSpeedCamera);
            testNetwork.get(0).getConnection(testNetwork.get(2)).addSensor(demoPressurePad);
            testNetwork.get(0).getConnection(testNetwork.get(2)).addSpeedSign(110);
            testNetwork.get(0).getConnection(testNetwork.get(1)).addSpeedSign(100);

            testNetwork.get(0).reduceIncomingSpeedSigns();
        } catch (DuplicateSensorException | RouteNotFoundException e) {
            e.printStackTrace();
        }
//        for (int num : speedCameraData) {
//            System.out.println(num);
//        }
        TimedItemManager timedItemManager = TimedItemManager.getTimedItemManager();
        timedItemManager.registerTimedItem(demoPressurePad);
        timedItemManager.registerTimedItem(demoSpeedCamera);
        timedItemManager.oneSecond();
        return testNetwork;
